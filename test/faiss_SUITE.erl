...
basic_new_test(_Config) ->
    Dim = 128,
    {ok, Index} = faiss:new(Dim),
    128 = faiss:dimension(Index),
    true = faiss:is_trained(Index),
    0 = faiss:ntotal(Index),
...
index_factory_test(_Config) ->
    Dim = 64,
    {ok, Flat} = faiss:index_factory(Dim, <<"Flat">>),
    64 = faiss:dimension(Flat),
    true = faiss:is_trained(Flat),
    ok = faiss:close(Flat),

    {ok, FlatIP} = faiss:index_factory(Dim, <<"Flat">>, inner_product),
    64 = faiss:dimension(FlatIP),
    ok = faiss:close(FlatIP),
    ok.
...
add_search_test(_Config) ->
    ...
    {ok, DistBin, LabelBin} = faiss:search(Index, Query, K),
...
serialize_deserialize_test(_Config) ->
    ...
    >>,
    ok = faiss:add(Index1, Vectors),
    2 = faiss:ntotal(Index1),

    %% Serialize
    {ok, Binary} = faiss:serialize(Index1),
    true = is_binary(Binary),
    true = byte_size(Binary) > 0,

    %% Deserialize
    {ok, Index2} = faiss:deserialize(Binary),
    8 = faiss:dimension(Index2),
    2 = faiss:ntotal(Index2),

    %% Search should give same results
    Query = <<1.0:32/float-native, 2.0:32/float-native, 3.0:32/float-native, 4.0:32/float-native,
              5.0:32/float-native, 6.0:32/float-native, 7.0:32/float-native, 8.0:32/float-native>>,
    {ok, Dist1, Lab1} = faiss:search(Index1, Query, 1),
    {ok, Dist2, Lab2} = faiss:search(Index2, Query, 1),
    Dist1 = Dist2,
    Lab1 = Lab2,

    ok = faiss:close(Index1),
    ok = faiss:close(Index2),
    ok.
...
file_io_test(Config) ->
    ...
    ok = faiss:add(Index1, Vectors),
    10 = faiss:ntotal(Index1),

    %% Write to file
    ok = faiss:write_index(Index1, Path),

    %% Read from file
    {ok, Index2} = faiss:read_index(Path),
    16 = faiss:dimension(Index2),
    10 = faiss:ntotal(Index2),
...
hnsw_test(_Config) ->
    Dim = 32,
    {ok, Index} = faiss:index_factory(Dim, <<"HNSW32">>),
    32 = faiss:dimension(Index),
    true = faiss:is_trained(Index),

    %% Add vectors
    Vectors = random_vectors(100, Dim),
    ok = faiss:add(Index, Vectors),
    100 = faiss:ntotal(Index),

    %% Search
    Query = random_vectors(1, Dim),
    {ok, _Dist, Labels} = faiss:search(Index, Query, 5),
